    ...
    <header className="border-b border-slate-800 bg-slate-950/70 backdrop-blur-md sticky top-0 z-40 px-6 py-4 flex items-center justify-between">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">Agreement & Contract Interpretation Workspace</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        {documents.length > 0 && (
          <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-lg px-3 py-1.5 shadow-soft-sm">
            <Clock className="w-4 h-4 text-slate-500" />
            <select
              value={selectedDocId}
              onChange={(e) => onSelectDocument(e.target.value)}
              className="bg-transparent text-sm font-medium text-slate-300 focus:outline-none cursor-pointer max-w-[200px]"
              disabled={isLoading}
            >
        ...
        <button
          onClick={() => window.dispatchEvent(new Event('reopen-floating-assistant'))}
          className="flex items-center gap-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 font-medium text-sm px-3.5 py-2 rounded-lg transition-all cursor-pointer shrink-0"
          title="Reopen Floating Assistant"
        >
        ...
        <button
          ...
          disabled={isLoading}
          className="flex items-center gap-2 bg-brand-600 hover:bg-brand-500 disabled:bg-brand-800 disabled:text-slate-400 text-white font-medium text-sm px-4 py-2 rounded-lg transition-all shadow-soft cursor-pointer shrink-0"
        >
